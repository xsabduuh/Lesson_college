/* =================================================================
   GRADES (النقاط والأداء) – نسخة متوافقة (مصححة)
================================================================= */
...
function renderGrades() {
  ...
  try {
    ...
    let allGrades = (DATA.grades || []).filter(g => g && g.cls === cls && g.subj === subj);
    ...
    console.error('renderGrades error:', err);
    sec.innerHTML = `<div class="panel">حدث خطأ في عرض النقاط: ${esc(err.message)}</div>`;
  }
}
...
function openGradeForm(id){
  let g = id ? DATA.grades.find(x=>x.id===id) : {};
  g = g || {}; // ← حماية: لو id غير موجود بالبيانات
...
function saveGrade(){
  ...
  const finalize=()=>{
    if(id){
      const target = DATA.grades.find(x=>x.id===id);
...
function deleteAllGrades(cls,subj){
  const subjLabel = (subjById(subj) || {}).label || subj;
  const clsLabel  = (clsById(cls) || {}).label || cls;
  if(!confirm(`حذف جميع نقاط ${subjLabel} لقسم ${clsLabel}؟`))return;
  DATA.grades=DATA.grades.filter(g=>!(g.cls===cls&&g.subj===subj));
  save(); toast('تم حذف الكل','success'); renderGrades();
}
