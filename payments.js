/* =================================================================
   ...
   (تم إصلاح مشكلة التعلق عند تغيير الحالة)
================================================================= */
...
function _monthSummary(cls, month) {
  ...
  stds.forEach(s => {
    ...
    else if (st === 'late')    { collected += amt; pending += fee - amt; nLate++; }
  ...
  return `
    ...
    <div class="panel" style="padding:10px 14px;margin-bottom:12px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px">
        <button class="btn btn-outline btn-sm" style="padding:8px 12px;${isFirst ? 'opacity:.4;pointer-events:none' : ''}"
          onclick="shiftPayMonth(-1)" ${isFirst ? 'disabled' : ''}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polyline points="15 18 9 12 15 6"/></svg>
        </button>
        <div style="text-align:center;flex:1">
          <div style="font-size:16px;font-weight:800;color:var(--text)">${monthName} ${y}</div>
          <div style="font-size:11px;color:var(--text-3);margin-top:2px">
            ${sm.nPaid} مدفوع · ${sm.nPartial} جزئي · ${sm.nUnpaid + sm.nLate} غير مدفوع
          </div>
        </div>
        <button class="btn btn-outline btn-sm" style="padding:8px 12px;${isLast ? 'opacity:.4;pointer-events:none' : ''}"
    ...
    <div style="display:flex;gap:8px;margin-bottom:14px">
      <button class="btn btn-green btn-sm" style="flex:1;gap:5px" onclick="markAllPaid('${cls}','${month}')">${IC.check}&nbsp; تحصيل الكل</button>
      <button class="btn btn-outline btn-sm" style="flex:1" onclick="resetMonthPayments('${cls}','${month}')">إعادة تعيين</button>
    </div>` : ''}
    ...
    ${stds.length === 0
      ...
      : `<div class="panel" style="padding:0;overflow:hidden">
          ${stds.map((s, i) => {
            ...
            const amt  = Number(rec?.paid || 0);
            return `
              <div style="padding:12px 14px;border-bottom:${i < stds.length-1 ? '1px solid var(--border)' : 'none'};display:flex;align-items:center;gap:10px;background:${st==='paid'?'rgba(23,138,111,.04)':st==='unpaid'?'rgba(220,53,69,.04)':'transparent'};">
                <div style="width:38px;height:38px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:800;background:${classBg(s.cls)};color:${classColor(s.cls)}">${s.name.charAt(0)}</div>
                <div style="flex:1;min-width:0">
                  <div style="font-size:14px;font-weight:700;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(s.name)}</div>
                  <div style="font-size:11px;color:var(--text-3);margin-top:2px">${st === 'partial' && amt ? `${amt} / ${fee} د.م` : `${fee} د.م/شهر`}</div>
                </div>
                <div style="display:flex;align-items:center;gap:7px;flex-shrink:0">
                  <span style="font-size:11px;font-weight:800;padding:4px 9px;border-radius:8px;background:${info.bg};color:${info.color};white-space:nowrap">${info.label}</span>
                  <button class="btn btn-outline btn-sm" style="padding:7px 10px" onclick="openPaymentForm('${s.id}','${month}')">${IC.edit}</button>
                </div>
              </div>`;
          }).join('')}
        </div>`}
    ...
    <div style="margin-top:12px;padding:12px 14px;border-radius:12px;background:var(--danger-light);border-right:3px solid var(--danger)">
      <div style="font-size:12px;font-weight:800;color:var(--danger);margin-bottom:5px">لم يُؤدُّوا (${sm.nUnpaid + sm.nLate})</div>
      <div style="font-size:13px;color:var(--text-2);line-height:1.8">
        ${stds.filter(s => { const st = (_payRec(s.id, month) || {}).status; return !st || st === 'unpaid' || st === 'none' || st === 'late'; }).map(s => esc(s.name)).join(' · ') || '—'}
      </div>
    </div>` : ''}
  `;
}
  ...
  return `
    ...
    <div class="panel" style="padding:12px 14px;margin-bottom:12px">
      <div style="display:flex;align-items:center;justify-content:space-between">
        <div>
          <div style="font-size:14px;font-weight:800;color:var(--text)">السنة الدراسية ${startY} - ${startY + 1}</div>
          <div style="font-size:11px;color:var(--text-3);margin-top:2px">${months.length} شهراً · اضغط على الشهر لتسجيل الأداء</div>
        </div>
        <div style="display:flex;flex-direction:column;gap:4px">
          ${[{ cls:'paid', label:'مدفوع' },{ cls:'partial', label:'جزئي' },{ cls:'late', label:'متأخر' }].map(k => `
            <div style="display:flex;align-items:center;gap:5px">
              <div class="pay-month-card ${k.cls}" style="width:14px;height:14px;padding:0;border-radius:4px;cursor:default"></div>
    ...
    ${stds.length === 0
      ...
      : stds.map(s => {
          ...
          const pct = months.length > 0 ? Math.round((nPaid + nPartial * 0.5) / months.length * 100) : 0;
          const pctColor = pct >= 75 ? 'var(--green)' : pct >= 40 ? 'var(--amber)' : 'var(--danger)';
          return `
            <div class="panel" style="padding:0;overflow:hidden;margin-bottom:10px">
              <div style="display:flex;align-items:center;gap:10px;padding:12px 14px;border-bottom:1px solid var(--border);background:var(--surface-2)">
                <div style="width:36px;height:36px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;background:${classBg(s.cls)};color:${classColor(s.cls)}">${s.name.charAt(0)}</div>
                <div style="flex:1;min-width:0">
                  <div style="font-size:13px;font-weight:800;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(s.name)}</div>
                  <div style="font-size:11px;color:var(--text-3);margin-top:1px">${fee} د.م/شهر · محصَّل: ${totalCollected.toLocaleString()} د.م</div>
                </div>
                <div style="font-size:13px;font-weight:800;font-family:var(--mono);color:${pctColor}">${pct}%</div>
              </div>
              <div class="progress-bar" style="height:4px;border-radius:0">
                <div class="progress-fill" style="width:${pct}%;height:100%;border-radius:0;background:${pctColor}"></div>
              </div>
...
function _statsViewHtml() {
  ...
  const rateColor = collectionRate >= 75 ? 'var(--green)' : collectionRate >= 40 ? 'var(--amber)' : 'var(--danger)';
  ...
  const monthlyData = pastMonths.map(mo => {
    ...
    stds.forEach(s => {
      ...
      else if (st === 'partial' || st === 'late') col += Number(rec.paid || 0);
    });
  ...
  const studentRows = stds.map(s => {
    ...
    pastMonths.forEach(mo => {
      ...
      else if (st === 'partial' || st === 'late') { col += Number(rec.paid || 0); pending += fee - Number(rec.paid || 0); nPartial++; }
      else                       { pending += fee; }
    });
    const pct = pastMonths.length > 0 ? Math.round((nPaid + nPartial * 0.5) / pastMonths.length * 100) : 0;
  ...
  return `
    ...
    <div class="panel" style="padding:14px;margin-bottom:14px">
      <div style="font-size:12px;font-weight:800;color:var(--text-3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:12px">التحصيل الشهري — ${startY}/${startY + 1}</div>
      <div style="display:flex;align-items:flex-end;gap:6px;height:80px">
        ${monthlyData.map(d => {
          const h = maxCol > 0 ? Math.max(Math.round((d.col / maxCol) * 70), d.col > 0 ? 4 : 0) : 0;
          return `
            <div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:4px">
              <div style="font-size:9px;font-weight:700;color:var(--text-3);font-family:var(--mono);opacity:${d.col > 0 ? 1 : 0}">${d.col > 999 ? Math.round(d.col/1000)+'k' : d.col}</div>
              <div style="width:100%;height:${h}px;min-height:${d.col > 0 ? 4 : 0}px;border-radius:5px 5px 0 0;background:${d.col > 0 ? 'var(--accent)' : 'var(--border)'};opacity:${d.col > 0 ? 1 : 0.4};align-self:flex-end;"></div>
              <div style="font-size:8px;color:var(--text-3);font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%">${d.label.slice(0, 3)}</div>
            </div>`;
        }).join('')}
      </div>
    </div>` : ''}
    ...
    ${stds.length === 0
      ? `<div class="panel">${emptyHtml('لا يوجد تلاميذ','أضف تلاميذ أولاً في قسم التلاميذ')}</div>`
      : `
      <div style="font-size:11px;font-weight:800;color:var(--text-3);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px">تفصيل التلاميذ (${pastMonths.length} شهر مدروس)</div>
      <div class="panel" style="padding:0;overflow:hidden">
        <div style="display:flex;align-items:center;gap:8px;padding:9px 14px;background:var(--surface-2);border-bottom:1px solid var(--border)">
        ...
        ${studentRows.map((s, i) => {
          ...
          return `
            <div style="padding:10px 14px;border-bottom:${i < studentRows.length-1 ? '1px solid var(--border)' : 'none'}">
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                <div style="flex:1;font-size:13px;font-weight:700;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(s.name)}</div>
                <div style="width:52px;text-align:right;font-size:12px;font-weight:800;font-family:var(--mono);color:var(--green)">${s.col.toLocaleString()}</div>
                <div style="width:52px;text-align:right;font-size:12px;font-weight:800;font-family:var(--mono);color:var(--danger)">${s.pending > 0 ? s.pending.toLocaleString() : '—'}</div>
                <div style="width:46px;text-align:center;font-size:12px;font-weight:800;font-family:var(--mono);color:#fff;background:${pctColor};padding:3px 0;border-radius:8px">${s.pct}%</div>
              </div>
              <div class="progress-bar"><div class="progress-fill" style="width:${s.pct}%;background:${pctColor}"></div></div>
            </div>`;
        }).join('')}
      </div>`}
  `;
...
function openPaymentForm(sid, month) {
  ...
  showSheet(`أداء ${monthName}`, `
    <div style="display:flex;align-items:center;gap:12px;padding:12px 14px;background:var(--surface-2);border-radius:10px;margin-bottom:16px">
      <div style="width:42px;height:42px;border-radius:50%;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:800;background:${classBg(s.cls)};color:${classColor(s.cls)}">${s.name.charAt(0)}</div>
      <div>
        <div style="font-size:14px;font-weight:800;color:var(--text)">${esc(s.name)}</div>
        <div style="font-size:12px;color:var(--text-3);margin-top:2px">${monthName} · الواجب: ${fee} د.م</div>
      </div>
    </div>

    <div class="field-row">
      <label>حالة الأداء <span style="color:var(--danger)">*</span></label>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:6px" id="pay-status-grid">
        ${statuses.map(st => {
          ...
          return `
            <button onclick="selectPayStatus('${st.val}')"
              data-status="${st.val}"
              style="padding:10px 8px;border-radius:10px;border:2px solid ${isSel ? info.color : 'var(--border)'};background:${isSel ? info.bg : 'var(--surface)'};color:${isSel ? info.color : 'var(--text-3)'};font-size:13px;font-weight:700;transition:all .15s;cursor:pointer;">
              ${st.label}
            </button>`;
        }).join('')}
      </div>
    ...
    <div class="field-row">
      ...
      <input class="field" type="number" id="xp-amount"
        ...
        value="${rec.paid != null ? rec.paid : ''}">
    </div>

    <div class="field-row">
      <label>ملاحظات</label>
      <input class="field" id="xp-note" placeholder="تاريخ الدفع، طريقة الدفع..." value="${esc(rec.note || '')}">
    </div>
...
function selectPayStatus(val) {
  ...
  // تعبئة المبلغ تلقائياً فقط في حالة "مدفوع كاملاً" إذا كان فارغاً
  if (val === 'paid') {
...
function savePayment() {
  ...
  const paidRaw = document.getElementById('xp-amount').value.trim();
  ...
  if (status === 'paid') {
    ...
    const entered = parseFloat(paidRaw);
    if (isNaN(entered) || entered < 0) {
      toast('أدخل مبلغاً صحيحاً', 'error');
      return;
    }
    paidAmt = Math.min(entered, fee);
  } else {
    paidAmt = 0;
  }
